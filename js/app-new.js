...
class TaskManager {
  ...
  openCategoryModal(categoryId = null) {
    ...
    if (categoryId) {
      ...
      if (category) {
        ...
        // Set color
        const colorInput = document.querySelector(`input[name="category-color"][value="${category.color}"]`);
        if (colorInput) {
  ...
  toggleLanguage() {
    ...
    document.getElementById('lang-text').textContent = newLang === 'en' ? '🇺🇸' : '🇸🇦';
  }
  ...
  updateCurrentDate() {
    ...
    if (dateElement) {
      ...
      dateElement.textContent = new Date().toLocaleDateString(this.settings.language, options);
    }
  }
